...
public final class RxJava2CallAdapterFactory extends CallAdapter.Factory {
    ...
    public CallAdapter<?, ?> get(Type returnType, Annotation[] annotations, Retrofit retrofit) {
        ...
        Rx2CacheConfig rx2CacheConfig = initRx2CacheConfig(annotations, responseType);

        return new RxJava2CallAdapter(responseType, scheduler, isAsync, isResult, isBody, isFlowable,
                isSingle, isMaybe, false, rx2CacheConfig);
    }

    private Rx2CacheConfig initRx2CacheConfig(Annotation[] annotations, Type responseType) {
        if (annotations == null) return null;

        Rx2CacheConfig.Builder builder = new Rx2CacheConfig.Builder();
        builder.setResponseType(responseType);

        for (Annotation annotation : annotations) {
            Log.d("ftd",annotation.toString());
            if (annotation instanceof Rx2Cache) {
                Rx2Cache rx2Cache = (Rx2Cache) annotation;
                builder.setCacheType(rx2Cache.CacheType());
            }
        }
